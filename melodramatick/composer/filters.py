...
class ComposerFilter(CustomEmptyLabelMixin, django_filters.FilterSet):
    ...
    group = django_filters.AllValuesFilter()
    complete = django_filters.ChoiceFilter(choices=((True, "Yes"), (False, "No")))
    gender = django_filters.ChoiceFilter(choices=GENDER_CHOICES)
    group = django_filters.ModelChoiceFilter(queryset=Group.objects.all(), lookup_expr='exact')
