...
## Example deployments
The live example ecosystem is maintained on https://github.com/haessar/melodramatick/tree/production, where currently Operatick, Balletick and Concertantick are linked as submodules. Guidance for promoting Melodramatick releases to `production`, as well as host-specific deployment information, can be found in the `deployments/` directory on the `production` branch. These live sites can each be accessed via https://melodramatick.com/.
